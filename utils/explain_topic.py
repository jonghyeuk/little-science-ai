...
def explain_topic(topic: str) -> list:
    ...
    - 실제 링크를 통해 확인 가능한 논문만 인용하고 논문 인용은 1개~3개정도, 논문제목을 보여주고 실제 확인가능한 링크를 보여줄것, 이 때 링크는 정상적으로 논문관련 페이지이어야함
    - 설명은 한국어로
    
    **확장 가능한 탐구 아이디어 제안** 섹션에서는:
    - 최소 3개 이상의 구체적인 연구 아이디어를 제시할 것
    - 각 아이디어의 제목은 별도의 글머리 기호(•)로 구분하여 명확히 표시하고 한줄에 1개씩 표시
    - 각 아이디어를 1-2문장으로 간결하게 설명하되 아이디어 제목 다음 줄에 표시, 아이디어를 설명할때 가운뎃점(·)으로 표시하고 설명할것
    - 각 아이디어의 설명이 줄이 넘어가면 설명은 들여쓰기하여 정렬할 것
    - 마지막에 "이 내용들을 토대로 위 검색창에 재검색을 진행해 보세요."라는 문구를 추가할 것
    """
