...
def explain_topic(topic: str) -> list:
    ...
    **다음 구조로 작성하되, 각 섹션은 자연스러운 문단 형태로 풍부하게 설명해:**
    ...
    ## 📊 **최근 논문 연구 동향**
    이 분야의 최근 연구 동향을 2개 정도 소개해주되, 논문 제목만 나열하지 말고 "이 연구에서는 이런 걸 발견했어요"라고 쉽게 설명해줘. 각 연구의 의미와 왜 중요한지를 자연스럽게 풀어서 설명해. **중요: DOI나 링크는 100% 확실하게 존재하고 접근 가능한 것만 포함하고, 링크를 남기고 논문 제목과 설명만 제공해줘.** 더 자세한 정보는 "이런 키워드로 검색하면 더 많은 자료를 찾을 수 있어요"라고 안내해줘.
    ...
    **이래 논문 밑에 아이디어를 선택하여 상세내용을 확인하세요요.**
