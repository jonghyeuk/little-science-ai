...
def explain_topic(topic: str) -> list:
    ...
    **다음 구조로 작성하되, 각 섹션은 자연스러운 문단 형태로 풍부하게 설명해:**
    ...
    ## 📚 **관련 연구 논문 & 자료**
    최근에 나온 중요한 연구들을 소개해주되, 논문 제목만 나열하지 말고 "이 연구에서는 이런 걸 발견했어요"라고 쉽게 설명해줘. 각 연구의 의미와 왜 중요한지를 자연스럽게 풀어서 설명하고, 더 자세히 알고 싶은 사람들을 위한 자료도 추천해줘. DOI나 링크는 실제 존재하는 것만 포함해.
