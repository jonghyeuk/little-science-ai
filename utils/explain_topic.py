...
def explain_topic(topic: str) -> list:
    ...
    **다음 구조로 작성하되, 각 섹션은 자연스러운 문단 형태로 풍부하게 설명해:**
    ...
    ---
    ## 📊 **최근 논문 연구 동향**
    이 분야의 최근 연구 동향을 2개 정도 소개해주되, 논문 제목만 나열하지 말고 "이 연구에서는 이런 걸 발견했어요"라고 쉽게 설명해줘. 각 연구의 의미와 왜 중요한지를 자연스럽게 풀어서 설명해. 
    
    **⚠️ 링크 제공 규칙:**
    - **절대로 구체적인 DOI나 직접 논문 링크(ieee, pubmed 등)를 만들어내지 마라**
    - 대신 논문 제목을 제공하고, Google Scholar 검색 링크를 만들어줘
    - 형식: "논문제목" ([Google Scholar에서 검색](https://scholar.google.com/scholar?q=논문제목))
    - 또는 일반적인 주제 검색 링크: ([관련 논문 더 찾아보기](https://scholar.google.com/scholar?q=주제+키워드))
    ---
    ## 🎯 **확장 가능한 탐구 아이디어**
