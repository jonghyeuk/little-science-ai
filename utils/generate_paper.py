...
def generate_research_paper(topic, research_idea, references=""):
    ...
    try:
        ...
        각 섹션 요구사항:
        - abstract: 연구목적과 예상결과 요약 (150-200단어, 최소 5-6문장)
         • 연구 배경 (1-2문장)
         • 연구 목적 (1문장)  
         • 연구 방법 요약 (1-2문장)
         • 예상 결과 (1-2문장)
        ...
        - introduction: 배경→문제→목적 순서 (300-400단어, 최소 3단락)
         • 1단락: 연구 주제의 중요성과 현재 상황 (4-5문장)
         • 2단락: 기존 연구의 한계점과 해결이 필요한 문제 (4-5문장)
         • 3단락: 본 연구의 목적과 가설, 접근 방법 (3-4문장)
        ...
        중요: abstract와 introduction은 반드시 지정된 최소 문장 수와 단락 구조를 지켜주세요.
        ...
        특별 주의사항:
        - 초록(abstract): 반드시 5-6문장으로, 각 요소(배경, 목적, 방법, 결과, 의의)를 포함하여 작성
        - 서론(introduction): 반드시 3개 단락으로 구성하고, 각 단락은 4-5문장으로 작성
        - 실험방법: "먼저 ~를 합니다. 다음으로 ~를 설정합니다" 친절한 구체적인 서술형
        - 결과내용,시각자료내용: 실험결과에 대한 데이타는 그림으로 그려지고 그것을 설명하는 자료가 시각자료임
        - 참고문헌: 간단한 한 문장만 작성

        초록과 서론은 특히 충실하게 작성해주세요. 너무 짧으면 안 됩니다.
        """
