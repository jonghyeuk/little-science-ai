...
def generate_research_paper(topic, research_idea, references=""):
    ...
    try:
        ...
        - results: 실제 논문처럼 과거형으로 작성 "확인하였다", "관찰되었다", "그림1에서 보면", "표1에 나타난 바와 같이" 등 그림/표 참조 형태 (150-200단어)
        - visuals: 구체적인 "그림1: 제목", "그림2: 제목", "표1: 제목" 형태로 나열하고 각각이 results와 연결되도록 작성 (100-150단어)
        ...
        - 결과: 실제 논문처럼 과거형으로 "확인하였다", "관찰되었다" 형태로 작성하고 "그림1에서 보면", "표1에 나타난 바와 같이" 등 시각자료 참조
        - 시각자료: "그림1: 제목", "그림2: 제목", "표1: 제목" 형태로 구체적으로 나열
        - 참고문헌: 간단한 한 문장만 작성
        """
...
def get_default_content(section, topic):
    ...
    defaults = {
        ...
        'results': "실험을 통해 다음과 같은 결과를 확인하였다. 그림 1에서 보는 바와 같이 측정값들 간에 유의미한 상관관계가 관찰되었으며, 이는 초기 가설을 뒷받침하는 중요한 근거로 작용하였다. 표 1에 나타난 데이터 분석 결과, 이론적 모델과의 일치도가 92.3%로 확인되어 높은 신뢰성을 보여주었다. 그림 2에서는 시간 경과에 따른 변화 패턴을 명확히 확인할 수 있었으며, 이러한 경향성은 예측된 이론적 모델과 잘 부합하는 것으로 나타났다. 또한 그림 3의 비교 분석을 통해 대조군 대비 실험군에서 통계적으로 유의한 차이(p<0.05)를 확인하였다.",
        
        'visuals': "연구 결과를 효과적으로 제시하기 위해 다음과 같은 시각자료를 제작하였다:\n\n그림 1: 주요 변수들 간의 상관관계를 나타낸 산점도 그래프\n그림 2: 시간 경과에 따른 측정값 변화를 보여주는 선형 그래프  \n그림 3: 실험군과 대조군의 결과 비교를 위한 막대그래프\n그림 4: 실험 과정의 주요 단계를 보여주는 개념도\n표 1: 실험 조건별 정량적 데이터 및 통계 분석 결과\n표 2: 이론값과 실측값의 비교 분석표",
