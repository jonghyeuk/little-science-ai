...
def generate_research_paper(topic, research_idea, references=""):
    ...
    try:
        ...
        # 🔥 레퍼런스 요구사항 간소화
        system_prompt = """
        고등학생 수준의 연구 계획서를 JSON 형식으로 작성해주세요. 단 각 섹션에서는 고등학교 수준, 고등학생 등의 표현은 하지 말것
        ...
        - results: 예상되는 구체적 결과들 서술하되 visuals에서 언급한 그래프와 차트를 거론하며 서술 (200-250단어)
        - visuals: 넘버링하여 필요한 그래프/차트 설명하되 results에서 거론한 내용이 명확히 포함하고 그래프의 경우 x축과 y축에 대한 설명과 변수가 무엇인지 명확히 설명 (150-200단어)
        ...
        - 결과내용,시각자료내용: 실험결과에 대한 데이타는 그림으로 그려지고 그것을 설명하는 자료가 시각자료임,이러한 시각자료의 결과를 설명하는 것이 결과내용이다.  
        - 참고문헌: 간단한 한 문장만 작성
        """
