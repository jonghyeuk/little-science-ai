...
def generate_research_paper(topic, research_idea, references=""):
    ...
    try:
        ...
        # 🔥 레퍼런스 요구사항 간소화
        system_prompt = """
        고등학교 수준의 연구 계획서를 JSON 형식으로 작성해주세요.

        🎓 **매우 중요: 반드시 고등학교 수준으로 작성하세요**
        - 고등학교 교과서에 나오는 수준의 용어와 개념만 사용
        - 대학교나 대학원 수준의 전문 용어는 절대 사용하지 마세요
        - 고등학생이 읽고 바로 이해할 수 있는 쉬운 문장으로 작성
        - 과학적 정확성은 유지하되, 표현은 고등학생 수준으로 쉽게

        반드시 이 JSON 형식만 사용:
        {"abstract": "초록내용", "introduction": "서론내용", "methods": "방법내용", "results": "결과내용", "visuals": "시각자료내용", "conclusion": "결론내용", "references": "검색가이드"}

        각 섹션 요구사항:
        - abstract: 연구목적과 예상결과 요약 (150-225단어) - 고등학교 수준 학술논문 형식
        - introduction: 배경→문제→목적 순서 (300-375단어) - 고등학교 수준 학술논문 형식
        ...
        실험방법 작성법:
        1단계: [제목] - (고등학교에서 구할 수 있는 장비/재료 간단히)
        ...
        위 내용으로 고등학교 수준의 학술 연구계획서를 JSON으로 작성해주세요.
        
        🎓 **반드시 고등학교 수준으로 작성하세요:**
        - 고등학생이 이해할 수 있는 쉬운 용어만 사용
        - 복잡한 전문 용어나 통계 개념은 피하세요
        - 고등학교 교과서 수준의 표현으로 작성
        
        주의사항:
        - 초록과 서론: 고등학교 수준 학술논문 형식으로 작성
        - 실험방법: 고등학교에서 구할 수 있는 재료/물품 목록을 포함하고 "먼저 ~를 합니다. 다음으로 ~를 설정합니다" 친절한 구체적인 서술형으로 상세하게
