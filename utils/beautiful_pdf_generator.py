...
# beautiful_pdf_generator.py 수정사항

# 1️⃣ generate_pdf 함수에서 이 부분을 찾으세요:
#    # 저장
#    output_path = os.path.join(OUTPUT_DIR, filename)
#    with suppress_fpdf_warnings():
#        pdf.output(output_path)

# 위의 pdf.output(output_path) 바로 앞에 이 한 줄 추가:
            # 🆕 논문 작성 가이드라인 페이지 추가
            pdf.add_guidelines_page()

# 2️⃣ ImprovedKoreanPDF 클래스 맨 끝(clean_text 메서드 뒤)에 이 두 메서드 추가:
